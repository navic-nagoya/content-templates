...
> **⚠️ 这是最危险的操作。** `style.css` 的 class 名是和线上商品的「合同」：任何已经上架的商品都在它的 HTML 里写死了这些 class，你改了就可能让线上商品样式崩。
...
2. 在生成器里用新 class 输出 HTML；
3. 保留旧 class 样式至少 **N 个月**，给运营时间逐步把老商品的片段替换过来；
4. 定期审计没人在用了再删旧 class。

### 如果必须改已有 class 的值

1. **先在本地改**，然后把 `style.css` 整体贴到一个临时 Shopify 商品说明，用线上真实商品数据 preview 一下；
2. 改的 diff 要小，一次只改一个维度（例如只改颜色），不要混着改间距和字号；
3. 通知所有运营同学：「下次你给商品刷新 Style Block 的时候，会出现 XXX 视觉变化」；
...
- [ ] 改 `style.css` 的话，是新增还是修改已有 class？如果是后者，评估过线上影响没？
- [ ] 新增的 UI 文案 / 模板默认文案，全部是日语吗？
- [ ] 新增的代码注释，全部是英文吗？
- [ ] `pnpm build` 通过了吗？
- [ ] 本地 `pnpm dev` 里每个控件都拨过一遍，同步更新正常吗？
- [ ] 复制出来的 HTML 粘到一个干净测试页，能渲染出和画廊里一样的效果吗？
- [ ] 要通知运营重新粘贴 Style Block 吗？要的话有提前说吗？
...
A：部分模板（像 Highlights）已经这么做了 —— 运营只要指定「项数」，列数由 CSS 自己算。如果新模板也符合"不关心具体几列"的场景，尽量用 `auto-fit` 更优雅。如果需要运营精准控制几列，用 `data-cols` 方案。
...
A：大概率是 Shopify 主题本身的 CSS 在覆盖。排查顺序：
1. 打开浏览器 DevTools，看具体被哪条规则覆盖；
2. 如果是主题的全局规则（如 `body img { max-width: 100% }` 一类），考虑在 `style.css` 里加 `.pd-section` 前缀提高优先级；
3. 不要用 `!important` 除非万不得已。
